...
def init():
    ...
    time.sleep(random.random())
    next_button = driver.find_element_by_xpath('/html/body/section/div/div/div[2]/div/form/div[2]/div/div/div/button')
    next_button.click()
...
def get_all_titles():
    ...
    for i in range(4): 
        next_table = driver.find_element_by_xpath('/html/body/div[4]/div[1]/table/thead/tr[1]/th[3]')
...
def get_titles():
    ...
    i = 1
    while i < 7:
        cal_row = driver.find_element_by_xpath(f"/html/body/div[4]/div[1]/table/tbody/tr[{i}]")
        titles = [str(td.get_attribute("title")) for td in cal_row.find_elements_by_tag_name("td")[1:-2]]
...
def any_slot(titles):
    for title in titles:
        try:
            if title not in ['Booked Out', 'Holiday']:
                return True
...
def loop():
    ...
    if any_slot(slots):
        pushbullet_message('Appointment slot', 'There is current an available slot!')
    else:
